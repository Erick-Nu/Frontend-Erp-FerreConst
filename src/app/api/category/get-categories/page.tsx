import { CategoryListReference } from '@/components/docs/CategoryListReference'
...
export default function ListarCategoriesPage() {
  ...
  const endpoint = {
    ...
    method: 'GET' as const,
    path: '/categories',
    definition: 'Lista categorías de forma paginada dentro de la empresa del usuario autenticado.',
    whenToUse:
      'Se usa cuando la aplicación necesita mostrar una tabla de categorías, buscar por nombre o descripción, o filtrar por estado. Al usarlo, el usuario obtiene una página de categorías y los datos de paginación necesarios para navegar el listado.',
    ...
    queryParams: [
      ...
      { name: 'search', type: 'string', required: false, description: 'Texto opcional para buscar por nombre o descripción de categoría.' },
      { name: 'status', type: 'string', required: false, description: 'Estado opcional. Solo acepta activo o inactivo.' },
    ],
    businessRules: [
      { title: 'Solo lista categorías de la empresa del usuario autenticado.' },
      ...
      { title: 'search busca coincidencias por ctgnombre o ctgriadescripcion.' },
      { title: 'status es un query param opcional y solo acepta activo o inactivo.' },
      { title: 'Si status no se envía, se excluyen las categorías con estado eliminado.' },
      { title: 'Las categorías se ordenan por fecha de registro descendente.' },
    ],
    expectedErrors: [
      ...
      { status: 500, title: 'Error finding categories', message: 'Ocurrió un error interno al consultar el listado de categorías.' },
    ],
    responseExample: `{
  "items": [
    {
  ...
  "totalPages": 1
    ...
    requestExample: `curl -X GET "${getApiBaseUrl()}/categories?page=1&pageSize=20&search=herramientas&status=activo" \
  -H "Authorization: Bearer <token>"`,
    notes: [
      ...
      'Usa search para buscar por nombre o descripción de categoría.',
      'Si no envías status, el listado devuelve categorías activas e inactivas, pero no eliminadas.',
      'Si no hay coincidencias, items se devuelve como un arreglo vacío.',
    ]
  }

  return <CategoryListReference moduleTitle={module.title} moduleSlug={module.slug} endpoint={endpoint} />
}
