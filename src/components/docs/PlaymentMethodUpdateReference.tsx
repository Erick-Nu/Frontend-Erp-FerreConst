...
type PlaymentMethodUpdateEndpoint = {
  slug: string
  title: string
  method: 'PATCH'
  path: string
...
type PlaymentMethodUpdateReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: PlaymentMethodUpdateEndpoint
}

export function PlaymentMethodUpdateReference({ moduleTitle, moduleSlug, endpoint }: PlaymentMethodUpdateReferenceProps) {
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
        ...
        <div className="mt-8 xl:hidden">
          <PlaymentMethodUpdatePreviewPanel endpoint={endpoint} />
        </div>
...
function PlaymentMethodUpdatePreviewPanel({ endpoint }: { endpoint: PlaymentMethodUpdateEndpoint }) {
