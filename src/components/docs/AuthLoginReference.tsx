...
export type LoginEndpoint = {
  ...
  body: FieldSpec[]
  businessRules: Array<{ title: string; description?: string; items?: string[] }>
...
type AuthLoginReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: LoginEndpoint
}

export function AuthLoginReference({ moduleTitle, moduleSlug, endpoint }: AuthLoginReferenceProps) {
    ...
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
          ...
          <div className="mt-4 flex items-start justify-between gap-6">
            <div className="space-y-3 sm:space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <TopBadge variant="method">{endpoint.method}</TopBadge>
              </div>
        ...
        <div className="mt-8 xl:hidden">
          <LoginPreviewPanel endpoint={endpoint} />
        </div>
...
function LoginPreviewPanel({ endpoint }: { endpoint: LoginEndpoint }) {
...
function TopBadge({ children, variant }: { children: ReactNode; variant: 'method' }) {
  const styles = {
    method: 'border-[var(--method-post-border)] bg-[var(--method-post-bg)] text-[var(--method-post)] font-black uppercase tracking-[0.05em]',
  }[variant]

  return (
    <span className={`inline-flex min-h-7 items-center justify-center rounded-[0.42rem] border-x px-3 py-1 text-[0.68rem] whitespace-nowrap ${styles}`} style={{ borderTopWidth: 0, borderBottomWidth: 0 }}>
      {children}
    </span>
  )
}
