...
type AlertEventsReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: AlertEventsEndpoint
}

export function AlertEventsReference({ moduleTitle, moduleSlug, endpoint }: AlertEventsReferenceProps) {
    ...
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
        ...
        <div className="mt-8 xl:hidden">
          <AlertPreviewPanel endpoint={endpoint} />
        </div>
...
function AlertPreviewPanel({ endpoint }: { endpoint: AlertEventsEndpoint }) {
  ...
  return (
    <aside className="space-y-5 sm:space-y-7 xl:sticky xl:top-24 xl:self-start">
      ...
      <CodeBlock title="Respuesta" label="text/event-stream" language="text" code={responseExample} maxHeight="30rem" />
    </aside>
  )
}
