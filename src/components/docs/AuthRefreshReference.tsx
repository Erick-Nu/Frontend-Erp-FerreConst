...
type AuthRefreshReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: RefreshEndpoint
}

export function AuthRefreshReference({ moduleTitle, moduleSlug, endpoint }: AuthRefreshReferenceProps) {
    ...
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
          ...
          <div className="mt-4 flex items-start justify-between gap-6">
            <div className="space-y-3 sm:space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <TopBadge>{endpoint.method}</TopBadge>
              </div>
        ...
        <div className="mt-8 xl:hidden">
          <RefreshPreviewPanel endpoint={endpoint} />
        </div>
...
function RefreshPreviewPanel({ endpoint }: { endpoint: RefreshEndpoint }) {
  ...
  return (
    <aside className="space-y-5 sm:space-y-7 xl:sticky xl:top-24 xl:self-start">
      ...
      <CodeBlock title="Respuesta" label="200 OK" language="json" code={responseExample} maxHeight="18rem" />
    </aside>
  )
}
...
function TopBadge({ children }: { children: ReactNode }) {
  return (
    <span
      className="inline-flex min-h-7 items-center justify-center rounded-[0.42rem] border-x border-[var(--method-post-border)] bg-[var(--method-post-bg)] px-3 py-1 text-[0.68rem] font-black uppercase whitespace-nowrap text-[var(--method-post)]"
