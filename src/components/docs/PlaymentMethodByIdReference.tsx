...
type PlaymentMethodByIdEndpoint = {
  slug: string
  title: string
  method: 'GET'
  path: string
...
type PlaymentMethodByIdReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: PlaymentMethodByIdEndpoint
}

export function PlaymentMethodByIdReference({ moduleTitle, moduleSlug, endpoint }: PlaymentMethodByIdReferenceProps) {
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
        ...
        <div className="mt-8 xl:hidden">
          <PlaymentMethodByIdPreviewPanel endpoint={endpoint} />
        </div>
...
function PlaymentMethodByIdPreviewPanel({ endpoint }: { endpoint: PlaymentMethodByIdEndpoint }) {
