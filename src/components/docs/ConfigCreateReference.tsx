...
type ConfigCreateReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: ConfigCreateEndpoint
}

export function ConfigCreateReference({ moduleTitle, moduleSlug, endpoint }: ConfigCreateReferenceProps) {
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
        ...
        <div className="mt-8 xl:hidden">
          <ConfigCreatePreviewPanel endpoint={endpoint} />
        </div>
...
function ConfigCreatePreviewPanel({ endpoint }: { endpoint: ConfigCreateEndpoint }) {
