...
type PlaymentMethodCreateEndpoint = {
  slug: string
  title: string
  method: 'POST'
  path: string
...
type PlaymentMethodCreateReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: PlaymentMethodCreateEndpoint
}

export function PlaymentMethodCreateReference({ moduleTitle, moduleSlug, endpoint }: PlaymentMethodCreateReferenceProps) {
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
        ...
        <div className="mt-8 xl:hidden">
          <PlaymentMethodCreatePreviewPanel endpoint={endpoint} />
        </div>
...
function PlaymentMethodCreatePreviewPanel({ endpoint }: { endpoint: PlaymentMethodCreateEndpoint }) {
  ...
  return (
    <aside className="space-y-5 sm:space-y-7 xl:sticky xl:top-24 xl:self-start">
      ...
      <CodeBlock title="Respuesta" label="201 Created" language="json" code={responseExample} maxHeight="30rem" />
    </aside>
  )
}
