...
type PlaymentMethodListEndpoint = {
  slug: string
  title: string
  method: 'GET'
  path: string
...
type PlaymentMethodListReferenceProps = {
  moduleTitle: string
  moduleSlug: string
  endpoint: PlaymentMethodListEndpoint
}

export function PlaymentMethodListReference({ moduleTitle, moduleSlug, endpoint }: PlaymentMethodListReferenceProps) {
  return (
    <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_440px] 2xl:grid-cols-[minmax(0,1fr)_500px]">
      <article className="min-w-0 pb-16 sm:pb-20">
        ...
        <div className="mt-8 xl:hidden">
          <PlaymentMethodListPreviewPanel endpoint={endpoint} />
        </div>
...
function PlaymentMethodListPreviewPanel({ endpoint }: { endpoint: PlaymentMethodListEndpoint }) {
